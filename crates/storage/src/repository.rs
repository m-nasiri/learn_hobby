...
pub struct NewDeckRecord {
    ...
    pub easy_days_enabled: bool,
    pub easy_day_load_factor: f32,
    pub easy_days_mask: u8,
...
impl NewDeckRecord {
    ...
    pub fn from_deck(deck: &Deck) -> Self {
        Self {
            ...
            min_interval_days: deck.settings().min_interval_days(),
            max_interval_days: deck.settings().max_interval_days(),
            easy_days_enabled: deck.settings().easy_days_enabled(),
            easy_day_load_factor: deck.settings().easy_day_load_factor(),
            easy_days_mask: deck.settings().easy_days_mask(),
...
impl DeckRepository for InMemoryRepository {
    async fn insert_new_deck(&self, deck: NewDeckRecord) -> Result<DeckId, StorageError> {
        let settings = DeckSettings::new(
            ...
            deck.min_interval_days,
            deck.max_interval_days,
            deck.easy_days_enabled,
            deck.easy_day_load_factor,
            deck.easy_days_mask,
